...
class Page extends CI_Controller {
    ...
    public function truck()  {
        ...
        $data['content'] = $this->load->view('Administrator/add_truck', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    public function insert_truck()  {
        $district = $this->input->post('truck');
        $query = $this->db->query("SELECT Truck_Name from tbl_truck where Truck_Name = '$truck'");
        ...
        if($query->num_rows() > 0){
            ...
            $data = array(
                "Truck_Name"          =>$this->input->post('truck', TRUE),
                "AddBy"                  =>$this->session->userdata("FullName"),
                "AddTime"                =>date("Y-m-d H:i:s")
                );
            
			if($this->mt->save_data('tbl_truck',$data))
			{
				$msg = true;
				echo json_encode($msg);
			}
    ...
    public function truckedit($id)  {
        ...
        $fld = 'Truck_SlNo';
        $data['selected'] = $this->Billing_model->select_by_id('tbl_truck', $id,$fld);
        $data['content'] = $this->load->view('Administrator/edit/truck_edit', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    public function truckupdate(){
        $id = $this->input->post('id');
        $fld = 'Truck_SlNo';
            $data = array(
                "Truck_Name"                     =>$this->input->post('truck', TRUE),
            ...
            if($this->mt->update_data("tbl_truck", $data, $id,$fld)){
                $msg = true;
				echo json_encode($msg);
            }
            /* else {
    ...
    public function truckdelete(){
        $id = $this->input->post('deleted');
        $fld = 'Truck_SlNo';
        $this->mt->delete_data("tbl_truck", $id, $fld);
        //$this->load->view('Administrator/ajax/district');
    } 

    public function getTrucks(){
        $trucks = $this->db->query("select * from tbl_truck where status = 'a'")->result();
