...
class Sales extends CI_Controller
{
        ...
        try {
            ...
            foreach ($data->cart as $cartProduct) {
                $saleDetails = array(
                    ...
                    'SaleDetails_per_unit'      => $cartProduct->per_unit,
    ...
    public function getSales()
    {
        ...
        $sales = $this->db->query("
            ...
            br.Brunch_name
            from tbl_salesmaster sm
            left join tbl_truck t on t.Truck_SlNo = sm.truck_id
            left join tbl_customer c on c.Customer_SlNo = sm.SalseCustomer_IDNo
            left join tbl_employee e on e.Employee_SlNo = sm.employee_id
            left join tbl_brunch br on br.brunch_id = sm.SaleMaster_branchid
            where sm.SaleMaster_branchid = '$branchId'
...
<?php
    ...
    public function PartyNoCount($custoemrId)
    {
        $year = date('Y');
        $returnCount = $this->db->query("select * from tbl_salesmaster where SalseCustomer_IDNo = ? and Status = 'a' and year(SaleMaster_SaleDate) = ?", [$custoemrId, $year])->result();
